...
public class SpellExplosions extends HomSpell {
    ...
    public ItemStack onOffensiveMeleeRightClick(ItemStack par1ItemStack, World par2World,
        ...
        par2World.createExplosion(
                par3EntityPlayer,
                par3EntityPlayer.posX + Math.sin(yaw) * Math.cos(pitch) * (-distance),
                par3EntityPlayer.posY + par3EntityPlayer.getEyeHeight() + Math.sin(-pitch) * distance,
                par3EntityPlayer.posZ + Math.cos(yaw) * Math.cos(pitch) * distance,
                (float) (3),
                true);
        return par1ItemStack;
    }
    ...
    public ItemStack onEnvironmentalRightClick(ItemStack par1ItemStack, World par2World,
        ...
        for (int i = 0; i < 360; i += 36) {
            par2World.createExplosion(
                    par3EntityPlayer,
                    par3EntityPlayer.posX + Math.cos(i) * radius,
                    par3EntityPlayer.posY,
                    par3EntityPlayer.posZ + Math.sin(i) * radius,
                    (float) (2),
                    true);
        }
