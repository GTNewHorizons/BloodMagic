...
public class RitualEffectEllipsoid extends RitualEffect {
    ...
    public void performEffect(IMasterRitualStone ritualStone) {
        ...
        if (!(tile instanceof IInventory above) || above.getSizeInventory() < 3) {
            return;
        }

        ItemStack item1 = above.getStackInSlot(0);
        ItemStack item2 = above.getStackInSlot(1);
        ItemStack item3 = above.getStackInSlot(2);
        ...
        if (!(tile instanceof IInventory below)) {
            return;
        }
        ...
        while (slot < below.getSizeInventory()) {
            stack = below.getStackInSlot(slot);
            if (stack == null) {
                slot++;
                continue;
            }
        ...
        if (placedBlock == null || slot >= below.getSizeInventory()) {
            return;
        }
                ...
                while (k <= zSize) {
                    ...
                    if (placedBlock.placeBlockAt(stack, null, world, x + i, y + j, z + k, 0, 0, 0, 0, j1)) {
                        ...
                        if (stack.stackSize <= 0) {
                            below.setInventorySlotContents(slot, null);
                        }
