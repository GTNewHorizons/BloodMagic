...
public class SigilOfWind extends EnergyItems implements ArmourUpgrade, ISigil {
    ...
    public void addInformation(ItemStack item, EntityPlayer player, List<String> tooltip, boolean adv) {
        tooltip.add(StatCollector.translateToLocal("tooltip.sigilofwind.desc"));
    ...
    public ItemStack onItemRightClick(ItemStack item, World world, EntityPlayer player) {
        ...
        if (toggleSigil(item, world, player)) {
            player.addPotionEffect(new PotionEffect(AlchemicalWizardry.customPotionProjProt.id, 2, 1));
        }
